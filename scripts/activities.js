...
window.onload = function () {
  ...
  categoryList.onchange = selectionChanged;

  populateCategories();


//   TODO: Need to add on change and call function to display info on change of activity
//   const activityList = document.getElementById("activityList");
//   activityList.onchange = displayChangedActivity;


};

function getAllCategories() {
  return ["Adventures", "Arts & Crafts", "Museums", "Wine Tastings", "Other"];
}
...
function selectionChanged() {
  ...
  const activitiesFiltered = filterActivities(category);
  console.log(activitiesFiltered);

  for (let activity of activitiesFiltered) {
    const option = new Option(activity.name);
    activityList.appendChild(option);
  }
  const activity = document.getElementById("activityList").value;
  console.log("activity", activity)
  let activityToDisplay = displayActivity(activity, activitiesFiltered);
  console.log(activityToDisplay)

}
function displayActivity(activity, activitiesFiltered) {
  for (let selectedActivity of activitiesFiltered) {
    if (selectedActivity.name == activity) {
      return selectedActivity;
    }
  }
}
function displayChangedActivity(){
  document.getElementById(
    "touristDetail"
  ).innerText = `Name: ${activityToDisplay.name} `;
}
